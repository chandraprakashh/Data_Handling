...
# Visualize this with the block of two rows

# In the first place, we check, if the first positions of the two string match
# If they dont match, then we match we the second position
# If they Match we then match all the left position
# If the sub string does not match we move to next position
...
"""
re.search(expr,s) checks a string s for an occurrence of a substring 
which matches the regular expression expr

The first substring (from left), which satisfies this condition will be returned.

If a match has been possible, we get a so-called match object as a result, 
otherwise the value will be None.
 
"""


# The simplest matching of substring in a string using Regular Expression
...
import re
x = re.finditer(r" .at ","A cat and a rat can't be friends.")
print (x)
next(x)

[(m.start(0), m.end(0)) for m in re.finditer(r" .at ","A cat and a rat can't be friends.")]
